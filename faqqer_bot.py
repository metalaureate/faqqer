...
def query_openai_gpt(system,faq_avoidance_text,prompt):
    ...
    system=system + "\n\nDo not talk about the following topics:\n" +faq_avoidance_text+ "\n\nIf you do not know the answer with certainty, tell the user that their question will be forwarded to support staff for answering.\n\nIf the questions seems missing, remind the user that the format for interacting with you is /faqqer <question> in their preferred language. Give an example, e.g., /faqqer What is Tari Universe?"
    try:
        client = OpenAI()
...
def find_faq_answer(question):
    ...
    If the question is not in English, please interpret the question correctly and translate the answer correctly into the user's language, using idioms appropriate to the technical subject matter. 
    Question:  %s
